...
@section("page-title","Transfer Receive Report")
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form method="get" action="{{route('receive-transfer-report.get')}}">
                                    @csrf
                                    <div class="row">
                                        <div class="col-md-2">
                                            ...
                                            <div class="form-group select2-parent">
                                                ...
                                                <select
                                                    ...
                                                    @foreach($to_warehouses as $to_warehouse)
                                                        <option value="{{$to_warehouse->id}}" @if(request()->query('warehouse_ser') == $to_warehouse->id) selected @endif>{{ucwords($to_warehouse->name)}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                            ...
                                            <div class="form-group select2-parent">
                                                ...
                                                <select
                                                    ...
                                                    @foreach($warehouses as $warehouse)
                                                        <option value="{{$warehouse->id}}" @if(request()->query('to_warehouse_ser') == $warehouse->id) selected @endif>{{ucwords($warehouse->name)}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-2">
                                            <div class="form-group" style="padding-top: 33px;">
                                                ...
                                                <a href="{{route("receive-transfer-report.get")}}" class="btn btn-danger " type="reset" id="reset_btn">Reset</a>
                                            </div>
                                        </div>
                                    </div>
                                </form>
                            ...
                            <div class="card-body table-responsive" id="prin-table">
                                <span id="count_pan">Displaying transfer receive from {{ ($transfers->total()) ? $sl+1 : 0 }} to {{ $sl+$transfers->count() }} out of total {{ $transfers->total() }}</span>
